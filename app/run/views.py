...
class PostRNASeq(View):
    ...
    def post(request, *args, **kwargs):
        if 'run_post_rnaseq' in request.POST:
            ...
            tar_file("results.tar.gz", "output/")
            zip_file("results.zip", "output/")
            ...
            if result != 0:
                ...
                # return redirect(f'/run/download_prna_{run_id}/')
        ...
        if post_rnaseq is True:
            if result == 0:
                ...
                if gtf_file is not None:
                    ...
                    for file in glob.glob("./work/stage/*/*/*.gtf"):
                        file_path = id_path + file
                        if os.path.isfile(file):
                            copy_file(file_path, "./genes.gtf")
                ...
                # import functions and compress results
                from .tasks import tar_file, zip_file
                tar_file("results_post.tar.gz", "output/")
                zip_file("results_post.zip", "output/")
